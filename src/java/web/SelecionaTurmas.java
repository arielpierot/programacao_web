 ...
 * @author arielpierot
 */
@WebServlet(name = "SelecionaTurmas", urlPatterns = {"/SelecionaTurmas"})
public class SelecionaTurmas extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {            
            ...
            ResultSet resultSet = statement.executeQuery("SELECT turmas.id, turmas.codigo, disciplinas.nome as disciplina, disciplinas.carga_horaria, turmas.horario_id_1, turmas.horario_id_2, turmas.horario_id_3  FROM turmas LEFT JOIN disciplinas ON disciplinas.id = turmas.disciplina_id ORDER BY nome;");       
            ...
            while(resultSet.next()){
                saida = saida +"<tr>\n"
                       ...
                       +"<td>" +data_formatada(resultSet.getString("horario_id_1"))+"</td>\n"
                       +"<td>" +data_formatada(resultSet.getString("horario_id_2"))+"</td>\n"
                       +"<td>" +data_formatada(resultSet.getString("horario_id_3"))+"</td>\n"
                       +"<td> <div class='form-check'> <label class=\"form-check-label\">\n" +
                        "        <input class=\"form-check-input\" type=\"checkbox\" name='turma_id[]' id='turma_id' value=\'"+ resultSet.getString("id") +"'>\n" +
            ...
            String conteudo = 
                           ...
                           "                  <p class='card-category'>Selecione as turmas desejadas</p>\n" +
                           "                </div>\n" +
                           "                <div class='card-body'>" +
                           "                  <form action='ComprovanteSolicitacao' method='get'>\n" +
